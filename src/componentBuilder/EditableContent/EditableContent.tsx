...
const EditableContent: FC<EditableContentProps> = ({ className, warningClass, defaultText, maxCount = 100, tagHTML = 'div', onChange, onFocusing, onWarning }) => {
  ...
  // Handle
  const handleEditing = (e: React.MouseEvent<HTMLHeadingElement, MouseEvent>) => {
    setIsFocusing(true);
  }

  const handleChange = (e: ChangeEvent<HTMLTextAreaElement>) => {
    setText(e.target.value);
    if (isWarning === false && e.target.value.length > maxCount) {
      setIsWarning(true);
    }
    if (isWarning === true && e.target.value.length <= maxCount) {
      setIsWarning(false);
    }
  }
