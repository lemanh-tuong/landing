...
const thunkChangeProfileUser = ({ displayName, email, photoURL, password }: Partial<ActionChangeProfileUserPayload & { password: string }>): ThunkChangeProfileUser => async (dispatch, getState) => {
  ...
  Promise.all([
    ...
    dispatch(actionChangeProfileUser.success({
      displayName: displayName || authReducer.profile.displayName,
