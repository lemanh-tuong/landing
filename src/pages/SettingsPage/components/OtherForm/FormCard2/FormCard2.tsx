...
const FormChangeCard: FC<FormChangeCardProps> = ({ nowIndexSection, indexCard }) => {
  ...
  // Render
  const _renderSettingsBox = (cardProperty: CardProps, nowIndexCard: number) => {
    const { textCard, titleCard, colorText, colorTitleCard, alignTitleCard, alignText, iconImg, alignIcon } = cardProperty;
    return (
      <Form
        fields={[
          {
          ...
          {provided => (
            <div ref={provided.innerRef} {...provided.droppableProps} className={styles.inner} >
              <div className={styles.listCard}>
                {card2s && card2s.map((cardProperty: CardProps, index: number) => _renderLabel(cardProperty, index))}
              </div>
              <div className={styles.form}>
                {card2s && card2s.map((cardProperty: any, index: number) => formShown.nowIndexCard === index ? _renderSettingsBox(cardProperty, index) : null)}
              </div>
            </div>
          )}
        </Droppable>
