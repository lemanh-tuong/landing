          ...
          {provided => (
            <div ref={provided.innerRef} {...provided.droppableProps} className={styles.inner} >
              <div className={styles.listRate}>
                {rateList && rateList?.map((rateProperty: RateProps, index: number) => _renderLabel(rateProperty, index))}
                <Button onClick={handleAdd} shape='circle' size='large' style={{ marginTop: 10 }}>
                  <i className="fas fa-plus" />
                </Button>
              </div>
              <div className={styles.form}>
                {rateList && rateList.map((_rateProperty: any, index: number) => formShown.nowIndexRate === index ? _renderSettingsBox(index) : null)}
              </div>
            </div>
          )}
        </Droppable>
