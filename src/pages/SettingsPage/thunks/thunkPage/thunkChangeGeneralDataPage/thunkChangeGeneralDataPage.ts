...
import {
  actionChangeGeneralDataPage,
  ActionChangeGeneralDataPagePayload,
} from 'pages/SettingsPage/actions/actionPage/actionChangeGeneralDataPage/actionChangeGeneralDataPage';
import { createDispatchAction } from 'utils/functions/reduxActions';

type ThunkChangeGeneralDataPage = ThunkAction<typeof actionChangeGeneralDataPage>;

const thunkChangeGeneralDataPage = ({
  nowIndexPage,
  newPageName,
  newPathName,
  id,
  isHome,
}: ActionChangeGeneralDataPagePayload): ThunkChangeGeneralDataPage => async (dispatch, getState) => {
  ...
  const newData = isHome
    ? data.map((page, index) => {
        if (index === nowIndexPage) return { ...page, pageName: newPageName, pathName: newPathName, isHome: true, id: id };
        return { ...page, isHome: false };
      })
    : [
        ...data.slice(0, nowIndexPage),
        { ...data[nowIndexPage], pageName: newPageName, pathName: newPathName, isHome: isHome },
        ...data.slice(nowIndexPage + 1, data.length),
      ];
  try {
