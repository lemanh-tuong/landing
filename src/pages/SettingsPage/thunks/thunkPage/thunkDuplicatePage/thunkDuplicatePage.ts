...
const thunkDuplicatePage = ({ pathName, pageName, id, isHome, pathNameSourcePage }: ThunkDuplicatePageArg): ThunkDuplicatePage => async (
  dispatch,
  getState,
) => {
  ...
  if (pathNameSourcePage) {
    ...
    const newGeneralPageData = isHome
      ? data
          .map(page => {
            if (page.isHome) return { ...page, isHome: false };
            return { ...page };
          })
          .concat({ id, pathName, pageName, isHome: true })
      : data.concat({ id, pathName, pageName, isHome: false });
    try {
      await Promise.all([
